...
from training.datasets import Consistency, Consistency_val
...
def parse_arguments():
    ...
    parser.add_argument('-d', '--data_dir', default='/N/u/chen478/fall2018/apu_project/Pytorch-SiamFC/consistency_dataset/',
                        help="Full path to the directory containing the dataset")
    parser.add_argument('-e', '--exp_name', default='matching',
...
def main(args):
    ...
    if params.model == 'mobile':
        model = mdl.SiameseNet(mdl.Backbone(), corr_map_size=33)
    ...
    val_set = Consistency_val(dataset_dir,
    ...
    val_loader = DataLoader(val_set, batch_size=101,
    ...
    try:
        if args.mode == 'train':
            ...
            train_set = Consistency(dataset_dir,
                                    label_fcn=label_function,
                                    cxt_margin=params.context_margin,
                                    reference_size=params.reference_sz,
                                    search_size=params.search_sz,
        ...
        for i, sample in enumerate(dataloader):
            ...
            if i % params.save_summary_steps == 0:
                ...
                for (metric_name, metric_dict) in metrics.items():
                    ...
                    metric_value = metric_fcn(output_batch, labels_batch)
...
def evaluate(model, loss_fn, dataloader, metrics, params, args):
    ...
    with tqdm(total=params.eval_epoch_size) as progbar:
        ...
        for i, sample in enumerate(dataloader):
            ...
            if (tbx_index < args.summary_samples):
