...
#def crop_img(img_path, cy, cx, reg_s):
def crop_img(img, cy, cx, reg_s):
    ...
    #print('^^^^^^^^^^^')
    #imwrite('picture_out.jpg', cropped_img)
    #print(pads)
...
def resize_and_pad(cropped_img, out_sz, pads, reg_s=None, use_avg=True, resize_fcn=imresize):
    ...
    #newsize=(rz_cr_w, rz_cr_h)
    #cropped_img.resize(newsize)
    #rz_crop = np.array(cropped_img)
    #print(rz_crop.shape)
